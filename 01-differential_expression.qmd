...
usethis::use_git_config(user.name = "laurahuggon", user.email = "laura.huggon@kcl.ac.uk")
...
*No - stress pathways not upregulated.*
...
```{r}
# values that determine what genes are considered "differentially expressed"
foldchange=0.5
pval=0.05
...
*These values are what are normally used.*

*The actual paper used more stringent values as they have so many deferentially expressed genes.*
...
# enriched in brain vs lung
length(brainlung)
# enriched in brain vs liver
length(brainliver)
...
*People tend to use 1000 nt.*

```{r}
filt_atac_annot = atac_annot %>% filter(abs(Distance.to.TSS) <= 2000)
```

Are any of these ATAC peaks associated with our unique brain genes (brain_enriched) and liver_enriched?
...
*Length is the same before vs after - so, no.*
...
```{r}
# convert format of motif to plots
#| eval: true
dreme_out$motif %>% 
...
```{r}
# what TF does that motif represent?
#| eval: true
...
Is Nfe2l2/Nrf2 transcription factor brain-specific in our RNA-Seq data?
...
*It is highly expressed in the brain, but also in the lung and liver.*
